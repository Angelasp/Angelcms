...
using Angelcms.Payment.Alipay;
using Angelcms.Model;

namespace Angelcms.Web.api.payment.alipay
{
    public partial class index : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //=============================获得订单信息================================
            string order_no = AgeRequest.GetFormString("pay_order_no").ToUpper();
            decimal order_amount = AgeRequest.GetFormDecimal("pay_order_amount", 0);
            string user_name = AgeRequest.GetFormString("pay_user_name");
            string subject = AgeRequest.GetFormString("pay_subject");
            //以下收货人信息
            ...
            {
                Age_user_amount_log model = new BLL.user_amount_log().GetModel(order_no);
                if (model == null)
                {
            ...
            {
               Age_orders model = new BLL.orders().GetModel(order_no);
                if (model == null)
                {
            ...
            sParaTemp.Add("exter_invoke_ip", AgeRequest.GetIP()); ////获取客户端的IP地址

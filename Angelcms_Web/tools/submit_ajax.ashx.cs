...
namespace Angelcms.Web.tools
{
    ...
    {
        Age_siteconfig siteConfig = new BLL.siteconfig().loadConfig();
        Age_userconfig userConfig = new BLL.userconfig().loadConfig();
        public void ProcessRequest(HttpContext context)
        {
            //取得处事类型
            string action = AgeRequest.GetQueryString("action");
        ...
        private void comment_add(HttpContext context)
        {
            ...
            Age_article_comment model = new Age_article_comment();

            string code = AgeRequest.GetFormString("txtCode");
            int article_id = AgeRequest.GetQueryInt("article_id");
            string _content = AgeRequest.GetFormString("txtContent");
            //校检验证码
            ...
            //检查该文章是否存在
            Age_article artModel = new BLL.article().GetModel(article_id);
            if (artModel == null)
            {
            ...
            Age_users userModel = new Web.UI.BasePage().GetUserInfo();
            if (userModel != null)
            {
            ...
            model.user_ip = AgeRequest.GetIP();
        ...
        {
            int article_id = AgeRequest.GetQueryInt("article_id");
            int page_index = AgeRequest.GetQueryInt("page_index");
            int page_size = AgeRequest.GetQueryInt("page_size");
            int totalcount;
            StringBuilder strTxt = new StringBuilder();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                        Age_users userModel = new BLL.users().GetModel(Convert.ToInt32(dr["user_id"]));
                        if (userModel != null)
                        {
        ...
        {
            string username = AgeRequest.GetString("param");
            //如果为Null，退出
        ...
        {
            string code = AgeRequest.GetFormString("txtCode").Trim();
            string invitecode = AgeRequest.GetFormString("txtInviteCode").Trim();
            string username = Utils.ToHtml(AgeRequest.GetFormString("txtUserName").Trim());
            string password = AgeRequest.GetFormString("txtPassword").Trim();
            string email = Utils.ToHtml(AgeRequest.GetFormString("txtEmail").Trim());
            string mobile = Utils.ToHtml(AgeRequest.GetFormString("txtMobile").Trim());
            string userip = AgeRequest.GetIP();
            ...
            //检查默认组别是否存在
            Age_user_groups modelGroup = new BLL.user_groups().GetDefault();
            if (modelGroup == null)
            {
            ...
            {
                new BLL.user_amount_log().Add(model.id, model.user_name, AgeEnums.AmountTypeEnum.SysGive.ToString(), modelGroup.amount, "注册赠送金额", 1);
            }
            ...
            {
                //取得邮件模板内容
                Age_mail_template mailModel = new BLL.mail_template().GetModel("welcomemsg");
                if (mailModel != null)
                {
                    //替换标签
                    ...
                    //发送邮件
                    AgeMail.sendMail(siteConfig.emailsmtp, siteConfig.emailusername, siteConfig.emailpassword, siteConfig.emailnickname,
            ...
            {
                context.Session[AgeKeys.SESSION_USER_INFO] = model;
                context.Session.Timeout = 45;

                //防止Session提前过期
                Utils.WriteCookie(AgeKeys.COOKIE_USER_NAME_REMEMBER, "Agecms", model.user_name);
                Utils.WriteCookie(AgeKeys.COOKIE_USER_PWD_REMEMBER, "Agecms", model.password);
        ...
        private string verify_invite_reg(string user_name, string invite_code)
        {
            ...
            Age_user_code codeModel = codeBll.GetModel(invite_code);
            if (codeModel == null)
            {
        ...
        private string verify_email(Age_users userModel)
        {
            //生成随机码
            string strcode = Utils.GetCheckCode(20);
            BLL.user_code codeBll = new BLL.user_code();
            Age_user_code codeModel;
            //检查是否重复提交
            codeModel = codeBll.GetModel(userModel.user_name, AgeEnums.CodeEnum.RegVerify.ToString(), "d");
            if (codeModel == null)
            {
                codeModel = new Age_user_code();
                codeModel.user_id = userModel.id;
                codeModel.user_name = userModel.user_name;
                codeModel.type = AgeEnums.CodeEnum.RegVerify.ToString();
            ...
            //获得邮件内容
            Age_mail_template mailModel = new BLL.mail_template().GetModel("regverify");
            if (mailModel == null)
            {
            ...
            try
            {
                AgeMail.sendMail(siteConfig.emailsmtp, 
        ...
        {
            //检查用户是否登录
            Age_users model = new BasePage().GetUserInfo();
            if (model == null)
            {
            ...
            {
                int result = codeBll.GetCount("user_name='" + model.user_name + "' and type='" + AgeEnums.CodeEnum.Register.ToString() + "' and datediff(d,add_time,getdate())=0");
            ...
            //删除过期的邀请码
            codeBll.Delete("type='" + AgeEnums.CodeEnum.Register.ToString() + "' and status=1 or datediff(d,eff_time,getdate())>0");
            //随机取得邀请码
            string str_code = Utils.GetCheckCode(8);
            Age_user_code codeModel = new Age_user_code();
            codeModel.user_id = model.id;
            codeModel.user_name = model.user_name;
            codeModel.type = AgeEnums.CodeEnum.Register.ToString();
        ...
        {
            string username = AgeRequest.GetFormString("username");
            //检查是否过快
            ...
            Age_users model = new BLL.users().GetModel(username);
            if (model == null)
            {
        ...
        {
            string username = AgeRequest.GetFormString("txtUserName");
            string password = AgeRequest.GetFormString("txtPassword");
            string remember = AgeRequest.GetFormString("chkRemember");
            //检查用户名密码
            ...
            BLL.users bll = new BLL.users();
            Age_users model = bll.GetModel(username, password, userConfig.emaillogin, userConfig.mobilelogin, true);
            if (model == null)
            {
            ...
            {
                Utils.WriteCookie(AgeKeys.COOKIE_USER_NAME_REMEMBER, "Agecms", model.user_name, 43200);
                Utils.WriteCookie(AgeKeys.COOKIE_USER_PWD_REMEMBER, "Agecms", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
        ...
        private void user_oauth_bind(HttpContext context)
        {
            ...
            //检查用户名密码
            string username = AgeRequest.GetString("txtUserName");
            string password = AgeRequest.GetString("txtPassword");
            if (string.IsNullOrEmpty(username) || string.IsNullOrEmpty(password))
            {
                context.Response.Write("{\"status\": 0, \"msg\": \"温馨提示：请输入用户名或密码！\"}");
                return;
            ...
            //开始绑定
            Age_user_oauth oauthModel = new Age_user_oauth();
        ...
        private void user_oauth_register(HttpContext context)
        {
            ...
            context.Session.Timeout = 45;
            //记住登录状态，防止Session提前过期
            Utils.WriteCookie(AgeKeys.COOKIE_USER_NAME_REMEMBER, "Angelcms", model.user_name);
            Utils.WriteCookie(AgeKeys.COOKIE_USER_PWD_REMEMBER, "Angelcms", model.password);
            //写入登录日志
            new BLL.user_login_log().Add(model.id, model.user_name, "会员登录");
            //返回URL
            context.Response.Write("{\"status\":1, \"msg\":\"会员登录成功！\"}");
        ...
        private void user_info_edit(HttpContext context)
        {
            ...
            string email = Utils.ToHtml(AgeRequest.GetFormString("txtEmail"));
            string nick_name = Utils.ToHtml(AgeRequest.GetFormString("txtNickName"));
            string sex = AgeRequest.GetFormString("rblSex");
            string birthday = AgeRequest.GetFormString("txtBirthday");
            string telphone = Utils.ToHtml(AgeRequest.GetFormString("txtTelphone"));
            string mobile = Utils.ToHtml(AgeRequest.GetFormString("txtMobile"));
            string qq = Utils.ToHtml(AgeRequest.GetFormString("txtQQ"));
        ...
        private void user_avatar_crop(HttpContext context)
        {
            ...
            string fileName = AgeRequest.GetFormString("hideFileName");
            int x1 = AgeRequest.GetFormInt("hideX1");
            int y1 = AgeRequest.GetFormInt("hideY1");
            int w = AgeRequest.GetFormInt("hideWidth");
            int h = AgeRequest.GetFormInt("hideHeight");
            //检查是否图片
        ...
        private void user_password_edit(HttpContext context)
        {
            ...
            string oldpassword = AgeRequest.GetFormString("txtOldPassword");
            string password = AgeRequest.GetFormString("txtPassword");
            //检查输入的旧密码
        ...
        {
            string code = AgeRequest.GetFormString("txtCode");
            string username = AgeRequest.GetFormString("txtUserName").Trim();
            //检查用户名是否正确
            if (string.IsNullOrEmpty(username))
            {
            ...
            //检查用户信息
            BLL.users bll = new BLL.users();
            Age_users model = bll.GetModel(username);
            if (model == null)
            {
            ...
            string strcode = Utils.GetCheckCode(20);
            //获得邮件内容
            Age_mail_template mailModel = new BLL.mail_template().GetModel("getpassword");
            if (mailModel == null)
            {
            ...
            BLL.user_code codeBll = new BLL.user_code();
            Age_user_code codeModel;
            codeModel = codeBll.GetModel(username, AgeEnums.CodeEnum.RegVerify.ToString(), "d");
            if (codeModel == null)
            {
                codeModel = new Age_user_code();
                //写入数据库
                codeModel.user_id = model.id;
                codeModel.user_name = model.user_name;
                codeModel.type = AgeEnums.CodeEnum.Password.ToString();
                codeModel.str_code = strcode;
                codeModel.eff_time = DateTime.Now.AddDays(userConfig.regemailexpired);
                codeModel.add_time = DateTime.Now;
                codeBll.Add(codeModel);
        ...
        private void user_repassword(HttpContext context)
        {
            ...
            BLL.user_code codeBll = new BLL.user_code();
            Age_user_code codeModel = codeBll.GetModel(strcode);
            if (codeModel == null)
            {
            ...
            Age_users userModel = userBll.GetModel(codeModel.user_id);
            //执行修改操作
        ...
        private void user_message_delete(HttpContext context)
        {
            ...
            string check_id = AgeRequest.GetFormString("checkId");
            if (check_id == "")
            {
        ...
        private void user_message_add(HttpContext context)
        {
            ...
            string code = context.Request.Form["txtCode"];
            string send_save = AgeRequest.GetFormString("sendSave");
            string user_name = Utils.ToHtml(AgeRequest.GetFormString("txtUserName"));
            string title = Utils.ToHtml(AgeRequest.GetFormString("txtTitle"));
            string content = Utils.ToHtml(AgeRequest.GetFormString("txtContent"));
            //校检验证码
            string result = verify_code(context, code);
            if (result != "success")
            {
            ...
            //保存数据
            Age_user_message modelMessage = new Age_user_message();
            modelMessage.type = 2;
        ...
        private void user_point_convert(HttpContext context)
        {
            ...
            int amout = AgeRequest.GetFormInt("txtAmount");
            string password = AgeRequest.GetFormString("txtPassword");
            if (model.amount < 1)
            {
            ...
            //扣除金额
            int amountNewId = new BLL.user_amount_log().Add(model.id, model.user_name, AgeEnums.AmountTypeEnum.Convert.ToString(), amout * -1, "用户兑换积分", 1);
            //增加积分
            if (amountNewId < 1)
            {
            ...
            {
                //返还金额
                new BLL.user_amount_log().Add(model.id, model.user_name, AgeEnums.AmountTypeEnum.Convert.ToString(), amout, "用户兑换积分失败，返还金额", 1);
                context.Response.Write("{\"status\":0, \"msg\":\"转换过程中发生错误，请重新提交！\"}");
                return;
            }
        ...
        private void user_amount_recharge(HttpContext context)
        {
            ...
            decimal amount = AgeRequest.GetFormDecimal("order_amount", 0);
            int payment_id = AgeRequest.GetFormInt("payment_id");
            if (amount == 0)
            {
            ...
            new BLL.user_amount_log().Add(model.id, model.user_name, AgeEnums.AmountTypeEnum.Recharge.ToString(), order_no,
        ...
        {
            string goods_id = AgeRequest.GetFormString("goods_id");
            int goods_quantity = AgeRequest.GetFormInt("goods_quantity", 1);
            if (goods_id == "")
            {
            ...
            //查找会员组
            int group_id = 0;
            Age_users groupModel = new Web.UI.BasePage().GetUserInfo();
            if (groupModel != null)
            {
        ...
        {
            string goods_id = AgeRequest.GetFormString("goods_id");
            int goods_quantity = AgeRequest.GetFormInt("goods_quantity");
            if (goods_id == "")
            {
                context.Response.Write("{\"status\":0, \"msg\":\"您提交的商品参数有误！\"}");
                return;
        ...
        private void order_save(HttpContext context)
        {
            //获得传参信息
            int payment_id = AgeRequest.GetFormInt("payment_id");
            int express_id = AgeRequest.GetFormInt("express_id");
            string accept_name = Utils.ToHtml(AgeRequest.GetFormString("accept_name"));
            string post_code = Utils.ToHtml(AgeRequest.GetFormString("post_code"));
            string telphone = Utils.ToHtml(AgeRequest.GetFormString("telphone"));
            string mobile = Utils.ToHtml(AgeRequest.GetFormString("mobile"));
            string address = Utils.ToHtml(AgeRequest.GetFormString("address"));
            string message = Utils.ToHtml(AgeRequest.GetFormString("message"));
            //获取订单配置信息
            Age_orderconfig orderConfig = new BLL.orderconfig().loadConfig();
            ...
            Age_express expModel = new BLL.express().GetModel(express_id);
            if (expModel == null)
            {
            ...
            Age_payment payModel = new BLL.payment().GetModel(payment_id);
            if (payModel == null)
            {
                context.Response.Write("{\"status\":0, \"msg\":\"对不起，支付方式不存在或已删除！\"}");
                return;
            ...
            //检查用户是否登录
            Age_users userModel = new BasePage().GetUserInfo();
            if (userModel != null)
            {
                user_id = userModel.id;
            ...
            //检查购物车商品
            IList<Model.cart_items> iList = Angelcms.Web.UI.ShopCart.GetList(user_group_id);
            if (iList == null)
            {
            ...
            //统计购物车
            Model.cart_total cartModel = Angelcms.Web.UI.ShopCart.GetTotal(user_group_id);
            //保存订单=======================================================================
            Age_orders model = new Age_orders();
            ...
            model.add_time = DateTime.Now;
            //商品详细列表
            List<Age_order_goods> gls = new List<Age_order_goods>();
            foreach (Model.cart_items item in iList)
            {
                gls.Add(new Age_order_goods { goods_id = item.id, goods_title = item.title, goods_price = item.price, real_price = item.user_price, quantity = item.quantity, point = item.point });
            }
            ...
            //清空购物车
            Angelcms.Web.UI.ShopCart.Clear("0");
            //提交成功，返回URL
        ...
        private void order_cancel(HttpContext context)
        {
            ...
            //检查订单是否存在
            string order_no = AgeRequest.GetQueryString("order_no");
            Age_orders orderModel = new BLL.orders().GetModel(order_no);
        ...
        {
            int article_id = AgeRequest.GetInt("id", 0);
            int click = AgeRequest.GetInt("click", 0);
            int count = 0;
            if (article_id > 0)
            {
        ...
        {
            int attach_id = AgeRequest.GetInt("id", 0);
            int count = 0;
            if (attach_id > 0)
            {
        ...
        private string verify_code(HttpContext context,string strcode)
        {
            ...
            if (context.Session[AgeKeys.SESSION_CODE] == null)
            {
                return "{\"status\":0, \"msg\":\"对不起，验证码超时或已过期！\"}";
            }
            if (strcode.ToLower() != (context.Session[AgeKeys.SESSION_CODE].ToString()).ToLower())
            {
                return "{\"status\":0, \"msg\":\"您输入的验证码与系统的不一致！\"}";
            }
            context.Session[AgeKeys.SESSION_CODE] = null;
            return "success";
        }

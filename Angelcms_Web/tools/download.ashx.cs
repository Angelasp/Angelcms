...
namespace Angelcms.Web.tools
{
    ...
    public class download : IHttpHandler, IRequiresSessionState
    {
       Age_siteconfig siteConfig = new BLL.siteconfig().loadConfig();
        ...
        {
            int id = AgeRequest.GetQueryInt("id");
            //获得下载ID
            if (id < 1)
            {
            ...
            Age_article_attach model = bll.GetModel(id);
            //检查积分是否足够
            if (model.point > 0)
            {
                //检查用户是否登录
                ...
                //防止重复扣积分
                string cookie = Utils.GetCookie(AgeKeys.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString());
                if (cookie != model.id.ToString())
                {
                    ...
                    //写入Cookie
                    Utils.WriteCookie(AgeKeys.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString(), model.id.ToString(), 8640);
                }
            }
            //下载次数+1

...
namespace Angelcms.Web.tools
{
    ...
    public class admin_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            string column_name = AgeRequest.GetString("param");
        ...
        {
            string build_path = AgeRequest.GetString("param");
            string old_build_path = AgeRequest.GetString("old_build_path");
        ...
        {
            string channel_name = AgeRequest.GetString("param");
            string old_channel_name = AgeRequest.GetString("old_channel_name");
        ...
        {
            string new_name = AgeRequest.GetString("param");
            string old_name = AgeRequest.GetString("old_name");
        ...
        {
            string navname = AgeRequest.GetString("param");
            string old_name = AgeRequest.GetString("old_name");
        ...
        {
            string user_name = AgeRequest.GetString("param");
        ...
        {
            string filePath = AgeRequest.GetFormString("remotepath");
        ...
        {
            Age_manager adminModel = new ManagePage().GetAdminInfo(); //获得当前登录管理员信息
            if (adminModel == null)
            {
                return;
            }
            Age_manager_role roleModel = new BLL.manager_role().GetModel(adminModel.role_id); //获得管理角色信息
            if (roleModel == null)
            {
                return;
            }
            DataTable dt = new BLL.navigation().GetDataList(0, AgeEnums.NavigationEnum.System.ToString());
            this.get_navigation_childs(context, dt, 0, "", roleModel.role_type, roleModel.manager_role_values);

        }
        private void get_navigation_childs(HttpContext context, DataTable oldData, int parent_id, string parent_name, int role_type,List<Age_manager_role_value> ls)
        {
            ...
            for (int i = 0; i < dr.Length; i++)
            {
                ...
                if (isActionPass && role_type > 1)
                {
                    ...
                    foreach (string action_type_str in actionTypeArr)
                    {
                        ...
                        {
                            Age_manager_role_value modelt = ls.Find(p => p.nav_name == dr[i]["name"].ToString() && p.action_type == "Show");
                            if (modelt == null)
                            {
                                isActionPass = false;
                            }
        ...
        {
            //取得管理员登录信息
            Age_manager adminInfo = new Web.UI.ManagePage().GetAdminInfo();
            if (adminInfo == null)
            {
            ...
            string order_no = AgeRequest.GetString("order_no");
            string edit_type = AgeRequest.GetString("edit_type");
            if (order_no == "")
            {
            ...
            Age_orders model = bll.GetModel(order_no);
            if (model == null)
            {
            ...
            switch (edit_type.ToLower())
            {
                case "order_confirm": //确认订单
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AgeEnums.ActionEnum.Confirm.ToString()))
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AgeEnums.ActionEnum.Confirm.ToString(), "确认订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                    if (orderConfig.confirmmsg > 0)
                    {
                        switch (orderConfig.confirmmsg)
                        {
                            case 2: //邮件通知
                                ...
                                if (model.user_id > 0)
                                {
                                    Age_users userModel = new BLL.users().GetModel(model.user_id);
                                    ...
                                    //取得邮件模板内容
                                   Age_mail_template mailModel = new BLL.mail_template().GetModel(orderConfig.confirmcallindex);
                                    if (mailModel == null)
                                    {
                ...
                case "order_payment": //确认付款
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AgeEnums.ActionEnum.Confirm.ToString(), "确认付款订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                    if (orderConfig.confirmmsg > 0)
                    {
                        switch (orderConfig.confirmmsg)
                ...
                case "order_express": //确认发货
                    ...
                    string express_no = AgeRequest.GetFormString("express_no");
                    if (express_id == 0)
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AgeEnums.ActionEnum.Confirm.ToString(), "确认发货订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                        ...
                        switch (orderConfig.expressmsg)
                        {
                          ...
                          case 2: //邮件通知
                                ...
                                if (model.user_id > 0)
                                {
                                    ...
                                    //取得邮件模板内容
                                    Age_mail_template mailModel = new BLL.mail_template().GetModel(orderConfig.expresscallindex);
                                    if (mailModel == null)
                                    {
                                        context.Response.Write("{\"status\": 1, \"msg\": \"订单确认成功，但无法发送邮件<br/ >邮件通知模板不存在。\"}");
                                        return;
                ...
                case "order_complete": //完成订单=========================================
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name,  AgeEnums.ActionEnum.Confirm.ToString(), "确认交易完成订单号:" + model.order_no); //记录日志
                    #region 发送短信或邮件
                        ...
                        switch (orderConfig.completemsg)
                        {
                            case 2: //邮件通知
                                ...
                                if (model.user_id > 0)
                                {
                                    ...
                                    //取得邮件模板内容
                                    Age_mail_template mailModel = new BLL.mail_template().GetModel(orderConfig.completecallindex);
                                    if (mailModel == null)
                                    {
                                        context.Response.Write("{\"status\": 1, \"msg\": \"订单确认成功，但无法发送邮件<br/ >邮件通知模板不存在。\"}");
                                        return;
                ...
                case "order_cancel": //取消订单==========================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AgeEnums.ActionEnum.Cancel.ToString()))
                    {
                    ...
                    int check_revert1 = AgeRequest.GetFormInt("check_revert");
                    if (check_revert1 == 1)
                    {
                        //如果存在积分换购则返还会员积分
                        if (model.user_id > 0 && model.point < 0)
                        ...
                        {
                            new BLL.user_amount_log().Add(model.user_id, model.user_name, AgeEnums.AmountTypeEnum.BuyGoods.ToString(), model.order_amount, "取消订单退还金额，订单号：" + model.order_no);
                        }
                    }
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name,  AgeEnums.ActionEnum.Cancel.ToString(), "取消订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"取消订单成功！\"}");
                    break;
                case "order_invalid": //作废订单==========================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AgeEnums.ActionEnum.Invalid.ToString()))
                    {
                    ...
                    int check_revert2 = AgeRequest.GetFormInt("check_revert");
                    if (check_revert2 == 1)
                    {
                        //扣除购物赠送的积分
                        if (model.user_id > 0 && model.point > 0)
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name,  AgeEnums.ActionEnum.Invalid.ToString(), "作废订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"作废订单成功！\"}");
                    break;
                case "edit_accept_info": //修改收货信息====================================
                    //检查权限
                    if (!new BLL.manager_role().Exists(adminInfo.role_id, "order_list", AgeEnums.ActionEnum.Edit.ToString()))
                    {
                    ...
                    string accept_name = AgeRequest.GetFormString("accept_name");
                    string province = AgeRequest.GetFormString("province");
                    string city = AgeRequest.GetFormString("city");
                    string area = AgeRequest.GetFormString("area");
                    string address = AgeRequest.GetFormString("address");
                    string post_code = AgeRequest.GetFormString("post_code");
                    string mobile = AgeRequest.GetFormString("mobile");
                    string telphone = AgeRequest.GetFormString("telphone");
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name,  AgeEnums.ActionEnum.Edit.ToString(), "修改收货信息，订单号:" + model.order_no); //记录日志
                    context.Response.Write("{\"status\": 1, \"msg\": \"修改收货人信息成功！\"}");
                    break;
                case "edit_order_remark": //修改订单备注=================================
                    string remark = AgeRequest.GetFormString("remark");
                    if (remark == "")
                    {
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name,  AgeEnums.ActionEnum.Edit.ToString(), "修改订单备注，订单号:" + model.order_no); //记录日志
                ...
                case "edit_real_amount": //修改商品总金额================================
                    ...
                    decimal real_amount = AgeRequest.GetFormDecimal("real_amount", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name,  AgeEnums.ActionEnum.Edit.ToString(), "修改商品金额，订单号:" + model.order_no); //记录日志
                ...
                case "edit_express_fee": //修改配送费用==================================
                    ...
                    decimal express_fee = AgeRequest.GetFormDecimal("express_fee", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AgeEnums.ActionEnum.Edit.ToString(), "修改配送费用，订单号:" + model.order_no); //记录日志
                ...
                case "edit_payment_fee": //修改支付手续费=================================
                    ...
                    decimal payment_fee = AgeRequest.GetFormDecimal("payment_fee", 0);
                    ...
                    new BLL.manager_log().Add(adminInfo.id, adminInfo.user_name, AgeEnums.ActionEnum.Edit.ToString(), "修改支付手续费，订单号:" + model.order_no); //记录日志
        ...
        private int GetIsLoginAndIsStaticstatus()
        {
            ...
            if (adminInfo == null)
                return  -1;
            else if (!new BLL.manager_role().Exists(adminInfo.role_id, "app_builder_html", AgeEnums.ActionEnum.Build.ToString()))
                return -2;

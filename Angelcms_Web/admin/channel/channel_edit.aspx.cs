...
namespace Angelcms.Web.admin.channel
{
    public partial class channel_edit : Angelcms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_channel_list",AgeEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(); //绑定类别
                FieldBind(); //绑定扩展字段
        ...
        private string GetInherit(string page_type)
        {
            ...
            {
                case "index":
                    result = "Angelcms.Web.UI.Page.article";
                    break;
                case "category":
                    result = "Angelcms.Web.UI.Page.category";
                    break;
                case "list":
                    result = "Angelcms.Web.UI.Page.article_list";
                    break;
                case "detail":
                    result = "Angelcms.Web.UI.Page.article_show";
                    break;
            }
            return result;
        }
        ...
        private void ShowInfo(int _id)
        {
            ...
            Age_channel model = bll.GetModel(_id);
            ...
            if (model.channel_fields != null)
            {
                ...
                {
                   Age_channel_field modelt = model.channel_fields.Find(p => p.field_id == int.Parse(cblAttributeField.Items[i].Value)); //查找对应的字段ID
                    if (modelt != null)
                    {
        ...
        private bool DoAdd()
        {
            ...
            //添加频道扩展字段
            List<Age_channel_field> ls = new List<Age_channel_field>();
            for (int i = 0; i < cblAttributeField.Items.Count; i++)
            {
                if (cblAttributeField.Items[i].Selected)
                {
                    ls.Add(new Age_channel_field { field_id = Utils.StrToInt(cblAttributeField.Items[i].Value, 0) });
                }
            }
            ...
            if (itemTypeArr != null && itemNameArr != null && itemPageArr != null && itemTempletArr != null && itemRewriteArr != null)
            {
                    ...
                    && (itemPageArr.Length == itemTempletArr.Length) && (itemTempletArr.Length == itemRewriteArr.Length))
                {
                    ...
                    {
                        Age_url_rewrite urlModel = new Age_url_rewrite();
            ...
            AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加频道" + model.title); //记录日志
            return true;
        }
        #endregion
        ...
        private bool DoEdit(int _id)
        {
            ...
            for (int i = 0; i < cblAttributeField.Items.Count; i++)
            {
                if (cblAttributeField.Items[i].Selected)
                {
                    ls.Add(new Age_channel_field { channel_id = model.id, field_id = Utils.StrToInt(cblAttributeField.Items[i].Value, 0) });
                }
            }
            model.channel_fields = ls;
            ...
            AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改频道" + model.title); //记录日志
            return true;
        }
        #endregion
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action ==AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("site_channel_list",AgeEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("site_channel_list",AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;

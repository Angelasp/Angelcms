...
{
    public partial class channel_list : Angelcms.Web.UI.ManagePage
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "保存频道排序"); //记录日志
        ...
        {
            ChkAdminLevel("site_channel_list",AgeEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.channel bll = new BLL.channel();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    Age_channel model = bll.GetModel(id);
                    if (bll.Delete(id))
                    {
                        sucCount += 1;
                        //删除URL配置
            ...
            AddAdminLog(AgeEnums.ActionEnum.Delete.ToString(), "删除频道成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

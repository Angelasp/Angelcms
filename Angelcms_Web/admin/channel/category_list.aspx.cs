...
{
    public partial class category_list : Angelcms.Web.UI.ManagePage
    {
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "保存频道分类排序"); //记录日志
        ...
        {
            ChkAdminLevel("site_channel_category",AgeEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.channel_category bll = new BLL.channel_category();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    Age_channel_category model = bll.GetModel(id);
                    //删除成功后对应的目录及文件
                    if (bll.Delete(id))
                    {
                        sucCount += 1;
                        Utils.DeleteDirectory(siteConfig.webpath + AgeKeys.DIRECTORY_REWRITE_ASPX + "/" + model.build_path);
                        Utils.DeleteDirectory(siteConfig.webpath + AgeKeys.DIRECTORY_REWRITE_HTML + "/" + model.build_path);
                    }
                    else
                    {
                        errorCount += 1;
            ...
            AddAdminLog(AgeEnums.ActionEnum.Delete.ToString(), "删除频道分类成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

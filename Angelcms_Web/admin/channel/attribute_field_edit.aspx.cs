...
{
    public partial class attribute_field_edit : Angelcms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_channel_field",AgeEnums.ActionEnum.View.ToString()); //检查权限
        ...
        private void ShowInfo(int _id)
        {
            ...
            Age_article_attribute_field model = bll.GetModel(_id);
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加扩展字段:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "修改扩展字段:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action ==AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("site_channel_field",AgeEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("site_channel_field",AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;

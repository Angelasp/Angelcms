...
namespace Angelcms.Web.admin.channel
{
    public partial class category_edit : Angelcms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_channel_category", AgeEnums.ActionEnum.View.ToString()); //检查权限
        ...
        private void ShowInfo(int _id)
        {
            ...
            Age_channel_category model = bll.GetModel(_id);
        ...
        private bool DoAdd()
        {
            ...
            {
                //更新一下域名缓存
                CacheHelper.Remove(AgeKeys.CACHE_SITE_HTTP_DOMAIN);
                AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加频道分类:" + model.title); //记录日志
                return true;
            }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                //更新一下域名缓存
                CacheHelper.Remove(AgeKeys.CACHE_SITE_HTTP_DOMAIN);
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改频道分类:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("site_channel_category", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("site_channel_category", AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;

...
namespace Angelcms.Web.admin.manager
{
    public partial class manager_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("manager_list", AgeEnums.ActionEnum.View.ToString()); //检查权限
                Age_manager model = GetAdminInfo(); //取得管理员信息
        ...
        private void ShowInfo(int _id)
        {
            ...
            Age_manager model = bll.GetModel(_id);
        ...
        private bool DoAdd()
        {
            Age_manager model = new Age_manager();
            BLL.manager bll = new BLL.manager();
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加管理员:" + model.user_name); //记录日志
                return true;
            }
            return false;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改管理员:" + model.user_name); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("manager_list", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("manager_list", AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;

...
namespace Angelcms.Web.admin.manager
{
    public partial class role_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("manager_role", AgeEnums.ActionEnum.View.ToString()); //检查权限
                RoleTypeBind(); //绑定角色类型
                NavBind(); //绑定导航
        ...
        {
            Age_manager model = GetAdminInfo();
        ...
        private void NavBind()
        {
            ...
            DataTable dt = bll.GetList(0, AgeEnums.NavigationEnum.System.ToString());
        ...
        private void ShowInfo(int _id)
        {
            ...
            Age_manager_role model = bll.GetModel(_id);
            ...
            if (model.manager_role_values != null)
            {
                for (int i = 0; i < rptList.Items.Count; i++)
                {
                    ...
                    {
                        Age_manager_role_value modelt = model.manager_role_values.Find(p => p.nav_name == navName && p.action_type == cblActionType.Items[n].Value);
                        if (modelt != null)
                        {
        ...
        {
            bool result = false;
            Age_manager_role model = new Age_manager_role();
            BLL.manager_role bll = new BLL.manager_role();
            ...
            //管理权限
            List<Age_manager_role_value> ls = new List<Age_manager_role_value>();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                string navName = ((HiddenField)rptList.Items[i].FindControl("hidName")).Value;
                CheckBoxList cblActionType = (CheckBoxList)rptList.Items[i].FindControl("cblActionType");
                for (int n = 0; n < cblActionType.Items.Count; n++)
                {
                    ...
                    {
                        ls.Add(new Age_manager_role_value { nav_name = navName, action_type = cblActionType.Items[n].Value });
                    }
                }
            }
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加管理角色:" + model.role_name); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                for (int n = 0; n < cblActionType.Items.Count; n++)
                {
                    if (cblActionType.Items[n].Selected == true)
                    {
                        ls.Add(new Age_manager_role_value { role_id = _id, nav_name = navName, action_type = cblActionType.Items[n].Value });
                    }
                }
            }
            model.manager_role_values = ls;
            ...
            if (bll.Update(model))
            {
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改管理角色:" + model.role_name); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("manager_role", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("manager_role", AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;

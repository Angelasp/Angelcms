...
namespace Angelcms.Web.admin.article
{

    public partial class article_edit : Angelcms.Web.UI.ManagePage
    {
        ...
        {
            this.channel_id = AgeRequest.GetQueryInt("channel_id");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string _action = AgeRequest.GetQueryString("action");
            ...
            if (!string.IsNullOrEmpty(_action) && _action == AgeEnums.ActionEnum.Edit.ToString())
            {
                this.action = AgeEnums.ActionEnum.Edit.ToString();//修改类型
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", AgeEnums.ActionEnum.View.ToString()); //检查权限
                ShowSysField(this.channel_id); //显示相应的默认控件
                GroupBind(""); //绑定用户组
                TreeBind(this.channel_id); //绑定类别
                if (action == AgeEnums.ActionEnum.Edit.ToString()) //修改
                {
                    ShowInfo(this.id);
                }
            }
        ...
        {
            List<Age_article_attribute_field> ls = new BLL.article_attribute_field().GetModelList(this.channel_id, "is_sys=0");
            if (ls.Count > 0)
            {
                field_tab_item.Visible = true;
                field_tab_content.Visible = true;
            }
            foreach (Age_article_attribute_field modelt in ls)
            {
                //创建一个dl标签
        ...
        {
            //查找该频道所选的默认字段
            List<Age_article_attribute_field> ls = new BLL.article_attribute_field().GetModelList(this.channel_id, "is_sys=1");
            foreach (Age_article_attribute_field modelt in ls)
            {
                //查找相应的控件，如找到则显示
            ...
            //查找该频道所开启的功能
            Age_channel channelModel = new BLL.channel().GetModel(_channel_id);
        ...
        private void ShowInfo(int _id)
        {
            ...
            Age_article model = bll.GetModel(_id);
            ...
            //扩展字段赋值
            List<Age_article_attribute_field> ls1 = new BLL.article_attribute_field().GetModelList(this.channel_id, "");
            foreach (Age_article_attribute_field modelt1 in ls1)
            {
            ...
            if (model.group_price != null)
            {
                for (int i = 0; i < this.rptPrice.Items.Count; i++)
                {
                    ...
                    foreach (Age_user_group_price modelt in model.group_price)
                    {
        ...
        private void GroupBind(string strWhere)
        {
            ...
            //检查该频道是否开启会员组价格
            Age_channel model = new BLL.channel().GetModel(this.channel_id);
        ...
        {
            bool result = false;
            Age_article model = new Age_article();
            BLL.article bll = new BLL.article();
            ...
            {
                List<Age_article_albums> ls = new List<Age_article_albums>();
                for (int i = 0; i < albumArr.Length; i++)
                {
                    ...
                    if (imgArr.Length == 3)
                    {
                        ...
                        {
                            ls.Add(new Age_article_albums { original_path = imgArr[1], thumb_path = imgArr[2], remark = remarkArr[i] });
                        }
                        else
                        {
                            ls.Add(new Age_article_albums { original_path = imgArr[1], thumb_path = imgArr[2] });
                        }
                    }
                }
                model.albums = ls;
            ...
            {
                List<Age_article_attach> ls = new List<Age_article_attach>();
                for (int i = 0; i < attachFileNameArr.Length; i++)
                {
                    ...
                    ls.Add(new Age_article_attach { file_name = attachFileNameArr[i], file_path = attachFilePathArr[i], file_size = fileSize, file_ext = fileExt, point = _point });
                }
                model.attach = ls;
            }
            #endregion
            ...
            #region 保存会员组价格==============
            List<Age_user_group_price> priceList = new List<Age_user_group_price>();
            for (int i = 0; i < rptPrice.Items.Count; i++)
            {
                ...
                priceList.Add(new Age_user_group_price { group_id = _groupid, price = _price });
            }
            ...
            if (bll.Add(model) > 0)
            {
                ...
                AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加" + this.channel_name + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (albumArr != null)
            {
                ...
                for (int i = 0; i < albumArr.Length; i++)
                {
                    ...
                    if (imgArr.Length == 3)
                    {
                        if (!string.IsNullOrEmpty(remarkArr[i]))
                        {
                            ls.Add(new Age_article_albums { id = img_id, article_id = _id, original_path = imgArr[1], thumb_path = imgArr[2], remark = remarkArr[i] });
                        }
                        else
                        {
                            ls.Add(new Age_article_albums { id = img_id, article_id = _id, original_path = imgArr[1], thumb_path = imgArr[2] });
                        }
                    }
                }
                model.albums = ls;
                ...
                && attachIdArr.Length > 0 && attachFileNameArr.Length > 0 && attachFilePathArr.Length > 0 && attachFileSizeArr.Length > 0 && attachPointArr.Length > 0)
            {
                ...
                for (int i = 0; i < attachFileNameArr.Length; i++)
                {
                    ...
                    string fileExt = Utils.GetFileExt(attachFilePathArr[i]);
                    int _point = Utils.StrToInt(attachPointArr[i], 0);
                    ls.Add(new Age_article_attach { id = attachId, article_id = _id, file_name = attachFileNameArr[i], file_path = attachFilePathArr[i], file_size = fileSize, file_ext = fileExt, point = _point, });
                }
                model.attach = ls;
            }
            #endregion
            ...
            for (int i = 0; i < rptPrice.Items.Count; i++)
            {
                ...
                decimal _price = Convert.ToDecimal(((TextBox)rptPrice.Items[i].FindControl("txtGroupPrice")).Text.Trim());
                priceList.Add(new Age_user_group_price { id = hidPriceId, article_id = _id, group_id = hidGroupId, price = _price });
            }
            model.group_price = priceList;
            #endregion
            ...
            if (bll.Update(model))
            {
                ...
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改" + this.channel_name + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;

...
{
    public partial class comment_edit : Angelcms.Web.UI.ManagePage
    {
        private int id = 0;
        private string channel_name = string.Empty; //频道名称
        protected Age_article_comment model = new Age_article_comment();
        ...
        {
            ChkAdminLevel("channel_" + this.channel_name + "_comment", AgeEnums.ActionEnum.Reply.ToString()); //检查权限
            BLL.article_comment bll = new BLL.article_comment();
            ...
            bll.Update(model);
            AddAdminLog(AgeEnums.ActionEnum.Reply.ToString(), "回复" + this.channel_name + "频道评论ID:" + model.id); //记录日志

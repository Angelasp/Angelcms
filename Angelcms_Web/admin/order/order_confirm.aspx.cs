...
namespace Angelcms.Web.admin.order
{
    public partial class order_confirm : Web.UI.ManagePage
    {
        ...
        {
            ChkAdminLevel("order_list", AgeEnums.ActionEnum.Confirm.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.orders bll = new BLL.orders();
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    Age_orders model = bll.GetModel(id);
                    if (model != null)
                    {
                        //检查订单是否使用在线支付方式
                        if (model.payment_status > 0)
            ...
            AddAdminLog(AgeEnums.ActionEnum.Confirm.ToString(), "确认订单成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志

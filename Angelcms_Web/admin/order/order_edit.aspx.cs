...
namespace Angelcms.Web.admin.order
{
    ...
    {
        private int id = 0;
        protected Age_orders model = new Age_orders();
        ...
        private void ShowInfo(int _id)
        {
            ...
            if (model.user_id > 0)
            {
                Age_users user_info = new BLL.users().GetModel(model.user_id);
                if (user_info != null)
                {
                    Age_user_groups group_info = new BLL.user_groups().GetModel(user_info.group_id);
                    if (group_info != null)
                    {

...
namespace Angelcms.Web.admin.order
{
    ...
    {
        private int id = 0;
        protected Age_payment model = new Age_payment();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
         ...
         //   if (!Page.IsPostBack)
         //   {
                ChkAdminLevel("order_payment", AgeEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo(this.id);
           // }
        }
        ...
        private bool DoEdit(int _id)
        {
            bool result = false;
            BLL.payment bll = new BLL.payment();
            Age_payment model = bll.GetModel(_id);
            ...
            if (bll.Update(model))
            {
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改支付方式:" + model.title); //记录日志
                result = true;
            }
        ...
        {
            ChkAdminLevel("order_payment", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
            if (!DoEdit(this.id))
            {

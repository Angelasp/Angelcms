...
namespace Angelcms.Web.admin.order
{
    public partial class order_config : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_config", AgeEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        private void ShowInfo()
        {
            ...
            Age_orderconfig model = bll.loadConfig();
        ...
        {
            ChkAdminLevel("order_config", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.orderconfig bll = new BLL.orderconfig();
            Age_orderconfig model = bll.loadConfig();
            try
            {
                ...
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改订单配置信息"); //记录日志

...
namespace Angelcms.Web.admin.order
{
    public partial class express_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_express", AgeEnums.ActionEnum.View.ToString()); //检查权限
        ...
        private void ShowInfo(int _id)
        {
            ...
            Age_express model = bll.GetModel(_id);
        ...
        private bool DoAdd()
        {
            Age_express model = new Age_express();
            BLL.express bll = new BLL.express();
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加配送方式:" + model.title); //记录日志
                return true;
            }
            return false;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改配送方式:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("order_express", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("order_express", AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;

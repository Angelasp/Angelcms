...
namespace Angelcms.Web.admin.users
{
    public partial class user_config : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_config", AgeEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        private void ShowInfo()
        {
            ...
            Age_userconfig model = bll.loadConfig();
        ...
        {
            ChkAdminLevel("user_config", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.userconfig bll = new BLL.userconfig();
            Age_userconfig model = bll.loadConfig();
            try
            {
                ...
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改用户配置信息"); //记录日志

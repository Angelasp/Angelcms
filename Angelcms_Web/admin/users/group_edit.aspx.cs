...
namespace Angelcms.Web.admin.users
{
    ...
    public partial class group_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_group", AgeEnums.ActionEnum.View.ToString()); //检查权限
        ...
        private void ShowInfo(int _id)
        {
            ...
            Age_user_groups model = bll.GetModel(_id);
        ...
        {
            bool result = false;
            Age_user_groups model = new Age_user_groups();
            BLL.user_groups bll = new BLL.user_groups();
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加用户组:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改用户组:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("user_group", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("user_group", AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;

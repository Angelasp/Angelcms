...
namespace Angelcms.Web.admin.settings
{
    public partial class nav_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("app_navigation_list", AgeEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(AgeEnums.NavigationEnum.System.ToString()); //绑定导航菜单
        ...
        {
            BLL.navigation bll = new BLL.navigation();
            Age_navigation model = bll.GetModel(_id);
        ...
        private bool DoAdd()
        {
            try
            {
                ...
                if (bll.Add(model) > 0)
                {
                    AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加导航信息:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        private bool DoEdit(int _id)
        {
            try
            {
                ...
                if (bll.Update(model))
                {
                    AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "修改导航信息:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("app_navigation_list", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("app_navigation_list", AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;

    ...
    public partial class url_rewrite_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == AgeEnums.ActionEnum.Edit.ToString())
            {
                ...
                this.urlName = AgeRequest.GetQueryString("name");
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_url_rewrite", AgeEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(); //绑定频道
        ...
        private void ShowInfo(string _urlName)
        {
            ...
            Age_url_rewrite model = bll.GetInfo(_urlName);
        ...
        {
            BLL.url_rewrite bll = new BLL.url_rewrite();
            Age_url_rewrite model = new Age_url_rewrite();
            ...
            if (bll.Add(model))
            {
                AddAdminLog(AgeEnums.ActionEnum.Add.ToString(), "添加URL配置信息:" + model.name); //记录日志
                return true;
            }
            return false;
        }
        ...
        private bool DoEdit(string _urlName)
        {
            ...
            if (bll.Edit(model))
            {
                AddAdminLog(AgeEnums.ActionEnum.Edit.ToString(), "修改URL配置信息:" + model.name); //记录日志
                return true;
            }
            return false;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AgeEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("site_url_rewrite", AgeEnums.ActionEnum.Edit.ToString()); //检查权限
            ...
            {
                ChkAdminLevel("site_url_rewrite", AgeEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;

...
namespace Angelcms.Web.admin.settings
{
    public partial class templet_list : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("app_templet_list", AgeEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(); //绑定频道分类
                RptBind(); //绑定模板
            }
        }
        ...
        private void RptBind()
        {
            ...
            foreach (DirectoryInfo dir in dirInfo.GetDirectories())
            {
                ...
                Age_template model = GetInfo(dir.FullName);
                if (model != null)
                {
        ...
        private Age_template GetInfo(string xmlPath)
        {
            Age_template model = new Age_template();
        ...
        {
            ChkAdminLevel("app_templet_list", AgeEnums.ActionEnum.Build.ToString()); //检查权限
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    AddAdminLog(AgeEnums.ActionEnum.Build.ToString(), "生成模板:" + skinName);//记录日志

...
namespace Angelcms.BLL
{
    ...
    public partial class channel
    {
        ...
        public bool Exists(string name)
        {
            ...
            if (DirPathExists(siteConfig.webpath + "/" + AgeKeys.DIRECTORY_REWRITE_ASPX + "/", name))
            {
                return true;
            }
            //与存在的频道名称是否同名
        ...
        public bool Update(Age_channel model)
        {
            ...
            //取得旧的数据
            Age_channel oldModel = dal.GetModel(model.id);
            //开始修改数据
            if (dal.Update(model))
            {
                //如果名称和标题发生改变则修改对应的导航
                ...
                {
                    Age_navigation navModel = new BLL.navigation().GetModel("channel_" + oldModel.name);
                    if (navModel != null)
                    {

...
namespace Angelcms.BLL
{
    ...
    {
        private readonly Age_siteconfig siteConfig = new siteconfig().loadConfig(); //获得站点配置信息
        ...
        /// <returns></returns>
        public Age_users GetModel(string user_name, string password, int emaillogin, int mobilelogin, bool is_encrypt)
        {
            //检查一下是否需要加密
            if (is_encrypt)
            {
        ...
        public bool Upgrade(int id)
        {
            ...
            Age_user_groups groupModel = new user_groups().GetUpgrade(model.group_id, model.exp);
            if (groupModel == null)
            {
                return false;
            }
            ...
            if (result > 0)
            {
                ...
                {
                    new BLL.user_amount_log().Add(model.id, model.user_name, AgeEnums.AmountTypeEnum.SysGive.ToString(), groupModel.amount, "升级赠送金额", 1);
                }
            }
            return true;
        }

...
namespace Angelcms.BLL
{
	...
	public partial class channel_category
	{
        ...
        public bool Exists(string build_path)
        {
            ...
            if (DirPathExists(siteConfig.webpath + "/" + AgeKeys.DIRECTORY_REWRITE_ASPX + "/", build_path))
            {
                return true;
            }
            //与频道名称是否同名
		...
		{
            Age_channel_category oldModel = dal.GetModel(model.id);
            if (dal.Update(model))
            {
                if (oldModel.build_path.ToLower() != model.build_path.ToLower())
                {
                    //更改频道分类对应的目录名称
                    Utils.MoveDirectory(siteConfig.webpath + AgeKeys.DIRECTORY_REWRITE_ASPX + "/" + oldModel.build_path,
                        siteConfig.webpath + AgeKeys.DIRECTORY_REWRITE_ASPX + "/" + model.build_path);
                    Utils.MoveDirectory(siteConfig.webpath + AgeKeys.DIRECTORY_REWRITE_HTML + "/" + oldModel.build_path,
                        siteConfig.webpath + AgeKeys.DIRECTORY_REWRITE_HTML + "/" + model.build_path);
                }
                //修改对应的导航
        ...
        public Dictionary<string, string> GetCategoryDirs()
        {
            ...
            List<Age_channel_category> modelList = DataTableToList(GetList(0, "", "sort_id asc,id desc").Tables[0]);
            foreach (Age_channel_category model in modelList)
            {
        ...
        /// 获得数据列表
        /// </summary>
        public List<Age_channel_category> DataTableToList(DataTable dt)
        {
            List<Age_channel_category> modelList = new List<Age_channel_category>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Age_channel_category model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Age_channel_category();

...
namespace Angelcms.Web.UI
{
    public class HtmlBuilder
    {
        ...
        Age_siteconfig config = new BLL.siteconfig().loadConfig();//站点配置
        Age_channel modelchanel = new Age_channel();//频道实体类
        ...
        protected internal Age_siteconfig siteConfig = new BLL.siteconfig().loadConfig();
        public HtmlBuilder()
        {
            //构造函数
        }
        ...
        {
            string lang = AgeRequest.GetQueryString("lang");
            string name = AgeRequest.GetQueryString("name");
            string type = AgeRequest.GetQueryString("type");
            ...
            List<Age_url_rewrite> ls = (!string.IsNullOrEmpty(type)) ? bll.GetList(name, type) : bll.GetList(name);
            string linkurl = string.Empty;
            sbjson.Append("[");
            if (type == "indexlist")
            {
                #region 针对特殊需求
                List<Age_url_rewrite> ls2 = (!string.IsNullOrEmpty("list")) ? bll.GetList(name, "list") : bll.GetList(name, "list");
                foreach (Age_url_rewrite modeltrewrite2 in ls2)
                {
            ...
            else
            {
                ...
                if (type == "feedback")
                {
                    linkurl = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_ASPX, lang, "feedback.aspx");
                    string HTMLPattern = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension("feedback.aspx", config.staticextension)); //替换扩展名
                    sbjson.AppendFormat(urlstr, config.webpath, lang, linkurl, HTMLPattern);
                }
                else if (type == "linkindex")
                {
                    linkurl = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_ASPX, lang, "link.aspx");
                    string HTMLPattern = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension("link.aspx", config.staticextension)); //替换扩展名
                    sbjson.AppendFormat(urlstr, config.webpath, lang, linkurl, HTMLPattern);
                }
                else
                {

                    foreach (Age_url_rewrite modeltrewrite in ls)
                    {
                        if (modeltrewrite.url_rewrite_items.Count > 0)
                        {
                            if (modeltrewrite.channel == string.Empty || modeltrewrite.channel == name)
                            {
                                ...
                                foreach (Model.url_rewrite_item item in modeltrewrite.url_rewrite_items)
                                {
                                    ...
                                    {
                                        linkurl = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_ASPX, lang, modeltrewrite.page);
                                        string HTMLPattern = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(item.pattern, config.staticextension)); //替换扩展名
                                        if (sbjson.ToString().Length > 1)
                                            sbjson.Append(",");
                                        sbjson.AppendFormat(urlstr, config.webpath, lang, linkurl, HTMLPattern);
        ...
        private string GetArticleIndexUrlList(string lang, string channelname, string page, string pattern, string path, string querystring, int pagesize)
        {
            StringBuilder sburl = new StringBuilder();
                    ...
                    for (int q = 1; q <= pageindex; q++)
                    {
                        ...
                        string linkurl = string.Format("{0}/{1}/{2}?{3}", AgeKeys.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                        string HTMLPattern = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, "0", q), config.staticextension)); //替换扩展名
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    for (int q = 1; q <= pageindex; q++)
                    {
                        ...
                        string linkurl = string.Format("{0}/{1}/{2}?{3}", AgeKeys.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                        string HTMLPattern = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, dt.Rows[i]["id"].ToString(), q), config.staticextension)); //替换扩展名
                        if (!string.IsNullOrEmpty(sburl.ToString()))
                            sburl.Append(",");
                        sburl.AppendFormat(urlstr, config.webpath, lang, linkurl.Replace("&","^"), HTMLPattern);
                    }
        ...
        private string GetCategoryUrlList(string lang, string channelname, string page, string pattern, string path, string querystring, int pagesize)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string linkurl = string.Format("{0}/{1}/{2}?{3}", AgeKeys.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                    string HTMLPattern = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, dt.Rows[i]["id"].ToString()), config.staticextension)); //替换扩展名
                    if (!string.IsNullOrEmpty(sburl.ToString()))
                        sburl.Append(",");
        ...
        private string GetDetailUrlList(string lang, string channelname, string page, string pattern, string path, string querystring)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string linkurl = string.Format("{0}/{1}/{2}?{3}", AgeKeys.DIRECTORY_REWRITE_ASPX, lang, page, querystr);
                    string HTMLPattern = string.Format("{0}/{1}/{2}", AgeKeys.DIRECTORY_REWRITE_HTML, lang, Utils.GetUrlExtension(string.Format(path, strvalue), config.staticextension)); //替换扩展名
                    if (!string.IsNullOrEmpty(sburl.ToString()))
                        sburl.Append(",");
                    sburl.AppendFormat(urlstr, config.webpath, lang, linkurl, HTMLPattern);
                }
        ...
        {
            string lang = AgeRequest.GetQueryString("lang");
            string aspx_filename = AgeRequest.GetQueryString("aspx_filename");
            string catalogue = AgeRequest.GetQueryString("catalogue");
        ...
        private void CreateIndexHtml(string lang, string aspx_filename, string catalogue)
        {
            if (File.Exists(Utils.GetMapPath(config.webpath + aspx_filename.Substring(0, aspx_filename.IndexOf(".aspx") + 5))))
            {
                ...
                Age_channel_category modelchannelcategory = objchannel_category.GetModel(lang);

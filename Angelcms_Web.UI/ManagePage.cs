...
namespace Angelcms.Web.UI
{
    ...
    {
        protected internal Age_siteconfig siteConfig;
        ...
        {
            //如果Session为Null
            if (Session[AgeKeys.SESSION_ADMIN_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string adminname = Utils.GetCookie("AdminName", "Angelcms");
                string adminpwd = Utils.GetCookie("AdminPwd", "Angelcms");
                if (adminname != "" && adminpwd != "")
                {
                    BLL.manager bll = new BLL.manager();
                    Age_manager model = bll.GetModel(adminname, adminpwd);
                    if (model != null)
                    {
        ...
        /// 取得管理员信息
        /// </summary>
        public Age_manager GetAdminInfo()
        {
            if (IsAdminLogin())
            {
                Age_manager model = Session[AgeKeys.SESSION_ADMIN_INFO] as Age_manager;
                if (model != null)
                {
                    return model;
                }

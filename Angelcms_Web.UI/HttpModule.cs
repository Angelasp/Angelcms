...
namespace Angelcms.Web.UI
{
    ...
    public class HttpModule : System.Web.IHttpModule
    {
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            Age_siteconfig siteConfig = new BLL.siteconfig().loadConfig(); //获得站点配置信息

            //如果虚拟目录(不含安装目录)与站点根目录名相同则不需要重写
            if (IsDirExist(AgeKeys.CACHE_SITE_DIRECTORY, siteConfig.webpath, siteConfig.webpath, requestPath))
            {
                return;
            }
            ...
            if (siteConfig.staticstatus == 0)
            {
                #region 站点不开启重写处理方法=======================================
                //遍历URL字典，匹配URL页面部分
                foreach (Age_url_rewrite model in SiteUrls.GetSiteUrls().Urls)
                {
                    //查找到与页面部分匹配的节点
                    if (model.page == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        //如果该页面属于插件页则映射到插件目录，否则映射到频道分类目录
                        if (model.type == AgeKeys.DIRECTORY_REWRITE_PLUGIN)
                        {
                            context.RewritePath(string.Format("{0}{1}/{2}{3}",
                                siteConfig.webpath, AgeKeys.DIRECTORY_REWRITE_ASPX, AgeKeys.DIRECTORY_REWRITE_PLUGIN, requestPath));
                            return;
                        }
                        else
                        {
                            ...
                            context.RewritePath(string.Format("{0}{1}/{2}{3}",
                                siteConfig.webpath, AgeKeys.DIRECTORY_REWRITE_ASPX, defaultPath, requestPath));
                            return;
                        }
                    }
                }
            ...
            else
            {
                ...
                foreach (Age_url_rewrite model in SiteUrls.GetSiteUrls().Urls)
                {
                        ...
                        Utils.GetUrlExtension(model.page, siteConfig.staticextension) == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        ...
                        {
                            context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                siteConfig.webpath, AgeKeys.DIRECTORY_REWRITE_ASPX, AgeKeys.DIRECTORY_REWRITE_PLUGIN, model.page));
                            return;
                        }
                        else
                        {
                            ...
                            context.RewritePath(string.Format("{0}{1}/{2}/{3}", siteConfig.webpath, AgeKeys.DIRECTORY_REWRITE_ASPX, defaultPath, model.page));
                            return;
                        }
                    }
                    //遍历URL字典的子节点
                    foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                    {
                            ...
                            || (model.page == "index.aspx" && Regex.IsMatch(requestPath, string.Format("^{0}{1}$", patternPath, item.pattern), RegexOptions.None | RegexOptions.IgnoreCase)))
                        {
                            ...
                            if (siteConfig.staticstatus == 2  && 
                                (model.channel.Length > 0 || model.page.ToLower() == "index.aspx")) //频道页
                            {
                                if (domainPath == string.Empty && categoryPath != string.Empty)
                                {
                                    context.RewritePath(siteConfig.webpath + AgeKeys.DIRECTORY_REWRITE_HTML + Utils.GetUrlExtension(requestPath, siteConfig.staticextension, true));
                                }
                                else
                                {
                                    context.RewritePath(siteConfig.webpath + AgeKeys.DIRECTORY_REWRITE_HTML + "/" + defaultPath + Utils.GetUrlExtension(requestPath, siteConfig.staticextension, true));
                                }
                                return;
                            }
                            else if (model.type == AgeKeys.DIRECTORY_REWRITE_PLUGIN) //插件页
                            {
                                ...
                                context.RewritePath(string.Format("{0}{1}/{2}/{3}", siteConfig.webpath, AgeKeys.DIRECTORY_REWRITE_ASPX, AgeKeys.DIRECTORY_REWRITE_PLUGIN, model.page), 
                            ...
                            else //其它
                            {
                                string queryString = Regex.Replace(requestPath, string.Format("{0}{1}", patternPath, newPattern),
                                    item.querystring, RegexOptions.None | RegexOptions.IgnoreCase);
                                context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                    siteConfig.webpath, AgeKeys.DIRECTORY_REWRITE_ASPX, defaultPath, model.page), string.Empty, queryString);
                                return;
                            }
    ...
    public class SiteUrls
    {
        ...
        private SiteUrls()
        {
            ...
            List<Age_url_rewrite> ls = bll.GetList("");
            foreach (Age_url_rewrite model in ls)
            {
                foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                {
        ...
        {
            SiteUrls _cache = CacheHelper.Get<SiteUrls>(AgeKeys.CACHE_SITE_HTTP_MODULE);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(AgeKeys.CACHE_SITE_HTTP_MODULE, new SiteUrls(), Utils.GetXmlMapPath(AgeKeys.FILE_URL_XML_CONFING));
                    instance = CacheHelper.Get<SiteUrls>(AgeKeys.CACHE_SITE_HTTP_MODULE);
                }
            }
            return instance;
        }
    ...
    public class SiteDomains
    {
        ...
        {
            SiteDomains _cache = CacheHelper.Get<SiteDomains>(AgeKeys.CACHE_SITE_HTTP_DOMAIN);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    CacheHelper.Insert(AgeKeys.CACHE_SITE_HTTP_DOMAIN, new SiteDomains(), 10);
                    instance = CacheHelper.Get<SiteDomains>(AgeKeys.CACHE_SITE_HTTP_DOMAIN);
                }
            }
            return instance;
        }

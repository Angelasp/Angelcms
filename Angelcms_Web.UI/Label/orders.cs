...
namespace Angelcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        protected List<Age_article> get_order_goods_list(int order_id)
        {
            Age_orders model = new BLL.orders().GetModel(order_id);
            if (model == null)
            {
                return null;
            }
            List<Age_article> ls = new List<Age_article>();
            if (model.order_goods != null)
            {
                foreach (Age_order_goods modelt in model.order_goods)
                {
                    Age_article goodsModel = new BLL.article().GetModel(modelt.goods_id);
                    if (goodsModel != null)
                    {
                        ls.Add(goodsModel);
                    }
        ...
        protected bool get_order_payment_status(int order_id)
        {
            ...
            Age_payment payModel = new BLL.payment().GetModel(model.payment_id);
            if (payModel == null)
            {
                return false;
            }

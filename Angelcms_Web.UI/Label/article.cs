...
namespace Angelcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        {
            Age_article model = new BLL.article().GetModel(article_id);
        ...
        protected string get_article_field(string call_index, string field_name)
        {
            ...
            Age_article model = bll.GetModel(call_index);
            if (model != null && model.fields.ContainsKey(field_name))
            {
                return model.fields[field_name];
            }
        ...
        {
            Age_users userModel = GetUserInfo();
            if (userModel == null)
            {
                return -1;
            }
            ...
            if (model != null)
            {
                ...
                {
                    Age_user_group_price priceModel = model.group_price.Find(p => p.group_id == userModel.group_id);
                    if (priceModel != null)
                    {

...
namespace Angelcms.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        protected internal Age_userconfig uconfig = new BLL.userconfig().loadConfig();
        /// <summary>
        /// 父类的构造函数
        /// </summary>
        public BasePage()
        ...
        public string linkurl(string _key, params object[] _params)
        {
            ...
            Age_url_rewrite model = ht[_key] as Age_url_rewrite; //查找指定的URL配置节点
        ...
        {
            //如果Session为Null
            if (HttpContext.Current.Session[AgeKeys.SESSION_USER_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string username = Utils.GetCookie(AgeKeys.COOKIE_USER_NAME_REMEMBER, "Agecms");
                string password = Utils.GetCookie(AgeKeys.COOKIE_USER_PWD_REMEMBER, "Agecms");
                if (username != "" && password != "")
                {
                    BLL.users bll = new BLL.users();
                    Age_users model = bll.GetModel(username, password, 0, 0, false);
                    if (model != null)
                    {
                        HttpContext.Current.Session[AgeKeys.SESSION_USER_INFO] = model;
                        return true;
                    }
                }
            }
        ...
        /// 取得用户信息
        /// </summary>
        public Age_users GetUserInfo()
        {
            if (IsUserLogin())
            {
                Age_users model = HttpContext.Current.Session[AgeKeys.SESSION_USER_INFO] as Age_users;
                if (model != null)
                {
        ...
        private string GetFirstPath()
        {
            ...
            if (requestPath.StartsWith(config.webpath + AgeKeys.DIRECTORY_REWRITE_ASPX + "/"))
            {
                indexNum = (config.webpath + AgeKeys.DIRECTORY_REWRITE_ASPX + "/").Length;
            }

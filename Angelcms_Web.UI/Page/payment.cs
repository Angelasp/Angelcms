...
namespace Angelcms.Web.UI.Page
{
    public partial class payment : Web.UI.BasePage
    {
        ...
        protected Age_orderconfig orderConfig = new BLL.orderconfig().loadConfig(); //订单配置信息
        protected Age_users userModel;
        protected Age_orders orderModel;
        protected Age_user_amount_log amountModel;
        ...
        {
            //取得处事类型
            action = AgeRequest.GetString("action");
            order_no = AgeRequest.GetString("order_no");
            if (order_no.ToUpper().StartsWith("R")) //充值订单
            {
                order_type = AgeEnums.AmountTypeEnum.Recharge.ToString().ToLower();
            }
            else if (order_no.ToUpper().StartsWith("B")) //商品订单
            {
                order_type = AgeEnums.AmountTypeEnum.BuyGoods.ToString().ToLower();
            }

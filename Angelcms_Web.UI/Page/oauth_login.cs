...
namespace Angelcms.Web.UI.Page
{
    public partial class oauth_login : Web.UI.BasePage
    {
        ...
        {
            turl = Utils.GetCookie(AgeKeys.COOKIE_URL_REFERRER);
            ...
            Age_user_oauth oauthModel = new BLL.user_oauth().GetModel(HttpContext.Current.Session["oauth_name"].ToString(), HttpContext.Current.Session["oauth_openid"].ToString());
            if (oauthModel != null)
            {
                //检查用户是否存在
                Age_users model = new BLL.users().GetModel(oauthModel.user_name);
                if (model == null)
                {

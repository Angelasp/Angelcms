...
namespace Angelcms_DBUtility
{
    ...
    public partial class DbHelper
    {
                ...
                if (m_provider == null)
                {
                    lock (lockHelper)
                    {
                        if (m_provider == null)
                        {
                            try
                            {
                                // m_provider = (IDbProvider)Activator.CreateInstance(Type.GetType(string.Format("Discuz.Data.{0}Provider, Discuz.Data.{0}", DbConfigs.GetDbType), false, true));
                            }
                            catch
                            {
        ...
        private static bool UserSnapDatabase(string commandText)
        {
            ...
            if (commandText.StartsWith(DbConfigs.GetTablePrefix + "create"))
            {
                ...
                return false;
            else if (!commandText.StartsWith(DbConfigs.GetTablePrefix + "get"))
                return false;
        ...
        public static string GetRealConnectionString(string commandText, CommandType commandType)
        {
            ...
            if (EntLibConfigs.GetConfig() != null)
            {
                ...
                {
                    if ((commandType == CommandType.Text && commandText.IndexOf(DbConfigs.GetTablePrefix + "online]") > 0) ||

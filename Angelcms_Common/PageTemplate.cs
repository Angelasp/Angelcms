...
namespace Angelcms_Common
{
    ...
    public abstract class PageTemplate
    {
        ...
        static PageTemplate()
        {
            ...
            //嵌套模板标签(兼容)
            r[0] = new Regex(@"<!--angelcms{template ((skin=\\""([^\[\]\{\}\s]+)\\""(?:\s+))?)src=(?:\/|\\"")([^\[\]\{\}\s]+)(?:\/|\\"")(?:\s*)}-->", options);
            //模板路径标签(新增)
            r[1] = new Regex(@"<!--angelcms{templateskin((=(?:\\"")([^\[\]\{\}\s]+)(?:\\""))?)(?:\s*)}-->", options);
            //命名空间标签
            r[2] = new Regex(@"<!--angelcms{namespace (?:""?)([\s\S]+?)(?:""?)}-->", options);
            //C#代码标签
            r[3] = new Regex(@"<!--angelcms{csharp}-->([\s\S]+?)<!--angelcms{/csharp}-->", options);
            //loop循环(抛弃)
            r[4] = new Regex(@"<!--angelcms{loop ((\(([^\[\]\{\}\s]+)\) )?)([^\[\]\{\}\s]+) ([^\[\]\{\}\s]+)}-->", options);
            //foreach循环(新增)
            r[5] = new Regex(@"<!--angelcms{foreach(?:\s*)\(([^\[\]\{\}\s]+) ([^\[\]\{\}\s]+) in ([^\[\]\{\}\s]+)\)(?:\s*)}-->", options);
            //for循环(新增)
            r[6] = new Regex(@"<!--angelcms{for\(([^\(\)\[\]\{\}]+)\)(?:\s*)}-->", options);
            //if语句标签(抛弃)
            r[7] = new Regex(@"<!--angelcms{if (?:\s*)(([^\s]+)((?:\s*)(\|\||\&\&)(?:\s*)([^\s]+))?)(?:\s*)}-->", options);
            r[8] = new Regex(@"<!--angelcms{else(( (?:\s*)if (?:\s*)(([^\s]+)((?:\s*)(\|\||\&\&)(?:\s*)([^\s]+))*))?)(?:\s*)}-->", options);
            //if语句标签(新增)
            r[9] = new Regex(@"<!--angelcms{if\((([^\s]+)((?:\s*)(\|\||\&\&)(?:\s*)([^\s]+))?)\)(?:\s*)}-->", options);
            r[10] = new Regex(@"<!--angelcms{else(( (?:\s*)if\((([^\s]+)((?:\s*)(\|\||\&\&)(?:\s*)([^\s]+))?))?\))(?:\s*)}-->", options);
            //循环与判断结束标签(兼容)
            r[11] = new Regex(@"<!--angelcms{\/(?:loop|foreach|for|if)(?:\s*)}-->", options);
            //continue标签
            r[12] = new Regex(@"<!--angelcms{continue(?:\s*)}-->");
            //break标签
            r[13] = new Regex(@"<!--angelcms{break(?:\s*)}-->");
            //request标签
            r[14] = new Regex(@"(\{request\[([^\[\]\{\}\s]+)\]\})", options);
            //截取字符串标签
            r[15] = new Regex(@"(<!--angelcms{cutstring\(([^\s]+?),(.\d*?)\)}-->)", options);
            //url链接标签
            r[16] = new Regex(@"(<!--angelcms{linkurl\(([^\s]*?)\)}-->)", options);
            //声明赋值标签(兼容)
            r[17] = new Regex(@"<!--angelcms{set ((\(?([\w\.<>]+)(?:\)| ))?)(?:\s*)\{?([^\s\{\}]+)\}?(?:\s*)=(?:\s*)(.*?)(?:\s*)}-->", options);
            //数据变量标签
            r[18] = new Regex(@"(\{([^\[\]\{\}\s]+)\[([^\[\]\{\}\s]+)\]\})", options);
            //普通变量标签
            r[19] = new Regex(@"({([^\[\]/\{\}=:'\s]+)})", options);
            //时间格式转换标签
            r[20] = new Regex(@"(<!--angelcms{datetostr\(([^\s]+?),(.*?)\)}-->)", options);
            //整型转换标签
            r[21] = new Regex(@"(\{strtoint\(([^\s]+?)\)\})", options);
            //直接输出标签
            r[22] = new Regex(@"<!--angelcms{(?:write |=)(?:\s*)(.*?)(?:\s*)}-->", options);
        }
        ...
        public static string GetTemplate(string sitePath, string tempPath, string skinName, string templet, string fromPage, string inherit, string buildPath, int nest)
        {
            ...
            using (StreamReader objReader = new StreamReader(templetFullPath, Encoding.UTF8))
            {
                ...
                {
                    extNamespace.Append("\r\n<!--angelcms{@ Import namespace=\"" + m.Groups[1] + "\" }-->");
                    textOutput.Replace(m.Groups[0].ToString(), string.Empty);
                }
                //替换特殊标记
                textOutput.Replace("\r\n", "\r\r\r");
                textOutput.Replace("<!--angelcms{", "\r\r\n<!--angelcms{");
                textOutput.Replace("}-->", "}-->\r\r\n");
                textOutput.Replace("<!--angelcms{csharp}-->\r\r\n", "<!--angelcms{csharp}-->").Replace("\r\r\n<!--angelcms{/csharp}-->", "<!--angelcms{/csharp}-->");
                ...
                if (nest == 1)
                {
                    ...
                    string template = string.Format("<%@ Page Language=\"C#\" AutoEventWireup=\"true\" Inherits=\"{0}\" ValidateRequest=\"false\" %>\r\n" +
                        ...
                        "<%@ Import namespace=\"Angelcms_Common\" %>{1}\r\n\r\n" +
                        "<script runat=\"server\">\r\noverride protected void OnInit(EventArgs e)\r\n" +
                        "{{\r\n\r\n\t/* \r\n\t\tThis page was created by Angelcms Template Engine at {2}.\r\n\t\t" +
                        "本页面代码由Angelcms模板引擎生成于 {2}. \r\n\t*/\r\n\r\n\tbase.OnInit(e);\r\n\t" +
        ...
        private static string ConvertTags(int nest, string sitePath, string tempPath, string skinName, string inputStr)
        {
            ...
            {
                if (IsCodeLine)
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), "AgeRequest.GetString(\"" + m.Groups[2] + "\")");
                else
                    strTemplate = strTemplate.Replace(m.Groups[0].ToString(), string.Format("\" + AgeRequest.GetString(\"{0}\") + \"", m.Groups[2]));
            }
            #endregion

...
namespace Angelcms.DAL
{
	...
	public partial class article_albums
	{
        ...
        /// 获得数据列表
        /// </summary>
        public List<Age_article_albums> GetList(int article_id)
        {
            List<Age_article_albums> modelList = new List<Age_article_albums>();
            ...
            if (rowsCount > 0)
            {
                Age_article_albums model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Age_article_albums();
        ...
        /// </summary>
        public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Age_article_albums> models, int article_id)
        {
            StringBuilder idList = new StringBuilder();
            if (models != null)
            {
                foreach (Age_article_albums modelt in models)
                {
                    if (modelt.id > 0)
                    {
            ...
            {
                int rows = DbHelperMSSQL.ExecuteSql(conn, trans, "delete from " + databaseprefix + "article_albums where id=" + dr["id"].ToString()); //删除数据库
                if (rows > 0)
                {
        ...
        /// 删除相册图片
        /// </summary>
        public void DeleteFile(List<Age_article_albums> models)
        {
            if (models != null)
            {
                foreach (Age_article_albums modelt in models)

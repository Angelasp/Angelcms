...
namespace Angelcms.DAL
{
    ...
    public partial class orders
    {
        ...
        /// 增加一条数据,及其子表数据
        /// </summary>
        public int Add(Age_orders model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            if (model.order_goods != null)
            {
                ...
                foreach (Age_order_goods models in model.order_goods)
                {
        ...
        public bool UpdateField(string order_no, string strValue)
        {
            ...
            strSql.Append("update " + databaseprefix + "orders set " + strValue);
            strSql.Append(" where order_no='" + order_no + "'");
            int rowsAffected = DbHelperMSSQL.ExecuteSql(strSql.ToString());
            if (rowsAffected > 0)
            {
                return true;
            }
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Age_orders model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_orders GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds2.Tables[0].Rows.Count > 0)
                {
                    ...
                    List<Age_order_goods> models = new List<Age_order_goods>();
                    Age_order_goods modelt;
                    for (int n = 0; n < i; n++)
                    {
                        modelt = new Age_order_goods();
        ...
        /// 根据订单号返回一个实体
        /// </summary>
        public Age_orders GetModel(string order_no)
        {
            StringBuilder strSql = new StringBuilder();

...
namespace Angelcms.DAL
{
    ...
    public partial class navigation
    {
        ...
        public int Add(int parent_id, string nav_name, string title, string link_url, int sort_id, int channel_id, string action_type)
        {
            ...
            //开始赋值
            Age_navigation model = new Age_navigation();
            model.nav_type = AgeEnums.NavigationEnum.System.ToString();
            model.name = nav_name;
            model.title = title;
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Age_navigation model)
        {
            using (SqlConnection conn = new SqlConnection(DbHelperMSSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            Age_navigation model2 = GetModel(conn, trans, model.parent_id); //带事务
                        ...
                        //修改节点列表和深度
                        DbHelperMSSQL.ExecuteSql(conn, trans, "update " + databaseprefix + "navigation set class_list='" + model.class_list + "', class_layer=" + model.class_layer + " where id=" + model.id); //带事务
                        //如无异常则提交事务
                        trans.Commit();
                    }
                    catch
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Age_navigation model)
        {
            Age_navigation oldModel = GetModel(model.id); //旧的数据
            using (SqlConnection conn = new SqlConnection(DbHelperMSSQL.connectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (IsContainNode(model.id, model.parent_id))
                        {
                            ...
                            {
                                Age_navigation oldParentModel = GetModel(conn, trans, oldModel.parent_id); //带事务
                                class_list = oldParentModel.class_list + model.parent_id + ",";
                                class_layer = oldParentModel.class_layer + 1;
                            }
                            //先提升选中的父节点
                            DbHelperMSSQL.ExecuteSql(conn, trans, "update " + databaseprefix + "navigation set parent_id=" + oldModel.parent_id + ",class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + model.parent_id); //带事务
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_navigation GetModel(string nav_name)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_navigation GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,nav_type,name,title,sub_title,link_url,sort_id,is_lock,remark,parent_id,class_list,class_layer,channel_id,action_type,is_sys");
            strSql.Append(" from " + databaseprefix + "navigation ");
        ...
        /// 得到一个对象实体(重载，带事务)
        /// </summary>
        public Age_navigation GetModel(SqlConnection conn, SqlTransaction trans, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,nav_type,name,title,sub_title,link_url,sort_id,is_lock,remark,parent_id,class_list,class_layer,channel_id,action_type,is_sys");
            strSql.Append(" from " + databaseprefix + "navigation ");
        ...
        {
            //查找父节点信息
            Age_navigation model = GetModel(conn, trans, parent_id);
            if (model != null)
            {
                //查找子节点
                ...
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    DbHelperMSSQL.ExecuteSql(conn, trans, "update " + databaseprefix + "navigation set class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + id); //带事务

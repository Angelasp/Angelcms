...
namespace Angelcms.DAL
{
    ...
    public partial class article_category
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Age_article_category model)
        {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (model.imagesize_values != null)
                        {
                            ...
                            foreach (Age_article_images_size modelimagesize in model.imagesize_values)
                            {
                                ...
                                DbHelperMSSQL.ExecuteSql(conn, trans, strSqlimage.ToString(), parametersimages); 
                            }
                        }
                        ...
                        {
                            Age_article_category model2 = GetModel(conn, trans, model.parent_id); //带事务
                        ...
                        //修改节点列表和深度
                        DbHelperMSSQL.ExecuteSql(conn, trans, "update " + databaseprefix + "article_category set class_list='" + model.class_list + "', class_layer=" + model.class_layer + " where id=" + model.id); //带事务
                        trans.Commit();
                    }
                    catch
                    {
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Age_article_category model)
        {
            using (SqlConnection conn = new SqlConnection(DbHelperMSSQL.connectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            //查找旧数据
                            Age_article_category oldModel = GetModel(model.id);
                            //查找旧父节点数据
                            string class_list = "," + model.parent_id + ",";
                            int class_layer = 1;
                            if (oldModel.parent_id > 0)
                            {
                                Age_article_category oldParentModel = GetModel(conn, trans, oldModel.parent_id); //带事务
                                class_list = oldParentModel.class_list + model.parent_id + ",";
                                class_layer = oldParentModel.class_layer + 1;
                            }
                            //先提升选中的父节点
                            DbHelperMSSQL.ExecuteSql(conn, trans, "update " + databaseprefix + "article_category set parent_id=" + oldModel.parent_id + ",class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + model.parent_id); //带事务
                        ...
                        { 
                            ...
                            foreach (Age_article_images_size modelimagesize in model.imagesize_values)
                            {
                                ...
                                {
                                    strSqlimage.Append("update Age_article_images_size set ");
        ...
        public void Delete(int id)
        {
            ...
            DbHelperMSSQL.Query(strSql.ToString());
        }

        /// <summary>
        /// 得到一个对象实体
        /// </summary>
        public Age_article_category GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            Age_article_category model = new Age_article_category();
        ...
        /// 得到一个对象实体(重载，带事务)
        /// </summary>
        public Age_article_category GetModel(SqlConnection conn, SqlTransaction trans, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description from " + databaseprefix + "article_category ");
            strSql.Append(" where id=@id");
        ...
        {
            //查找父节点信息
            Age_article_category model = GetModel(conn, trans, parent_id);
            if (model != null)
            {
                //查找子节点
                string strSql = "select id from " + databaseprefix + "article_category where parent_id=" + parent_id;
                DataSet ds = DbHelperMSSQL.Query(conn, trans, strSql); //带事务
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    DbHelperMSSQL.ExecuteSql(conn, trans, "update " + databaseprefix + "article_category set class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + id); //带事务

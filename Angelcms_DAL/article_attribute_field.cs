...
namespace Angelcms.DAL
{
    ...
    public partial class article_attribute_field
    {
        ...
        public bool Exists(string column_name)
        {
            ...
            if (DbHelperMSSQL.Exists(strSql.ToString(), parameters) || DbHelperMSSQL.Exists(strSql2.ToString(), parameters2))
            {
                return true;
            }
            return false;
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Age_article_attribute_field model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Age_article_attribute_field model)
        {
        ...
        {
            //取得Model信息
            Age_article_attribute_field model = GetModel(id);
            //开始删除
            using (SqlConnection conn = new SqlConnection(DbHelperMSSQL.connectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        DataTable dt = DbHelperMSSQL.Query(conn, trans, strSql1.ToString(), parameters1).Tables[0];
                        ...
                        if (dt.Rows.Count > 0)
                        {
                            ...
                            {
                               Age_channel modelt = new channel(databaseprefix).GetModel(conn, trans, int.Parse(dr["channel_id"].ToString()));
                                if (modelt != null)
                                {
                                    new channel(databaseprefix).RehabChannelViews(conn, trans, modelt, modelt.name);
                                }
                            }
                        }
                        ...
                        //删除扩展字段表中一列
                        DbHelperMSSQL.ExecuteSql(conn, trans, "alter table " + databaseprefix + "article_attribute_value drop column " + model.name);
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_article_attribute_field GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            Age_article_attribute_field model = new Age_article_attribute_field();
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select Age_article_attribute_field.* ");

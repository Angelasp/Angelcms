...
namespace Angelcms.DAL
{
    public class article_images_size
    {
        ...
        /// </summary>
         public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Age_article_images_size> models, int category_id)
         {
             StringBuilder idList = new StringBuilder();
             if (models != null)
             {
                 foreach (Age_article_images_size modelt in models)
                 {
                     if (modelt.id > 0)
                     {
             ...
             {
                 DbHelperMSSQL.ExecuteSql(conn, trans, "delete from " + databaseprefix + "article_images_size where id=" + dr["id"].ToString()); //删除数据库           
             }
         }
         ...
         {
             List<Age_article_images_size > modelList = new List<Age_article_images_size>();
         ...
         /// 得到一个对象实体
         /// </summary>
         public Age_article_images_size DataRowToModel(DataRow row)
         {
             Age_article_images_size model = new Age_article_images_size();
             if (row != null)
             {

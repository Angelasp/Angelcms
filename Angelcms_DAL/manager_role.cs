...
namespace Angelcms.DAL
{
    ...
    public partial class manager_role
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Age_manager_role model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            {
                foreach (Age_manager_role_value modelt in model.manager_role_values)
                {
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Age_manager_role model)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_manager_role GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                #region 子表信息
                StringBuilder strSql2 = new StringBuilder();
                strSql2.Append("select id,role_id,nav_name,action_type from Age_manager_role_value ");
                ...
                {
                    List<Age_manager_role_value> models = new List<Age_manager_role_value>();
                    Age_manager_role_value modelt;
                    foreach (DataRow dr in ds2.Tables[0].Rows)
                    {
                        modelt = new Age_manager_role_value();
                        if (dr["id"].ToString() != "")
                        {

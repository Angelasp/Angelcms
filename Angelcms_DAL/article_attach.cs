...
namespace Angelcms.DAL
{
    ...
    public partial class article_attach
    {
        ...
        /// </summary>
        public void DeleteList(SqlConnection conn, SqlTransaction trans, List<Age_article_attach> models, int article_id)
        {
            StringBuilder idList = new StringBuilder();
            if (models != null)
            {
                foreach (Age_article_attach modelt in models)
                {
                    if (modelt.id > 0)
                    {
            ...
            {
                int rows = DbHelperMSSQL.ExecuteSql(conn, trans, "delete from " + databaseprefix + "article_attach where id=" + dr["id"].ToString()); //删除数据库
                if (rows > 0)
                {
        ...
        /// 删除附件文件
        /// </summary>
        public void DeleteFile(List<Age_article_attach> models)
        {
            if (models != null)
            {
                foreach (Age_article_attach modelt in models)
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_article_attach GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            Age_article_attach model = new Age_article_attach();
        ...
        /// 获得数据列表
        /// </summary>
        public List<Age_article_attach> GetList(int article_id)
        {
            List<Age_article_attach> modelList = new List<Age_article_attach>();
            ...
            if (rowsCount > 0)
            {
                Age_article_attach model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Age_article_attach();

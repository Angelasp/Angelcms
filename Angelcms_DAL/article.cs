...
namespace Angelcms.DAL
{
    ...
    public partial class article
    {
        ...
        public int GetClick(int id)
        {
            ...
            strSql.Append(" where id=" + id);
            string str = Convert.ToString(DbHelperMSSQL.GetSingle(strSql.ToString()));
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Model.Age_article model)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            if (model.albums != null)
            {
                ...
                foreach (Age_article_albums modelt in model.albums)
                {
            ...
            if (model.attach != null)
            {
                ...
                foreach (Age_article_attach modelt in model.attach)
                {
            ...
            if (model.group_price != null)
            {
                ...
                foreach (Age_user_group_price models in model.group_price)
                {
            ...
            DbHelperMSSQL.ExecuteSqlTranWithIndentity(sqllist);
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Age_article model)
        {
                        ...
                        if (model.attach != null)
                        {
                            ...
                            foreach (Age_article_attach modelt in model.attach)
                            {
                                ...
                                if (modelt.id > 0)
                                {
                                    ...
                                    DbHelperMSSQL.ExecuteSql(conn, trans, strSql4.ToString(), parameters4);
                                }
                                else
                                {
                                    strSql4.Append("insert into " + databaseprefix + "article_attach(");
                        ...
                        if (model.group_price != null)
                        {
                            ...
                            foreach (Age_user_group_price modelt in model.group_price)
                            {
                                ...
                                if (modelt.id > 0)
                                {
                                    ...
                                    DbHelperMSSQL.ExecuteSql(conn, trans, strSql5.ToString(), parameters5);
                                }
                                else
                                {
                                    strSql5.Append("insert into " + databaseprefix + "user_group_price(");
        ...
        {
            //取得相册MODEL
            List<Age_article_albums> albumsList = new DAL.article_albums(databaseprefix).GetList(id);
            //取得附件MODEL
            List<Age_article_attach> attachList = new DAL.article_attach(databaseprefix).GetList(id);
            ...
            int rowsAffected = DbHelperMSSQL.ExecuteSqlTran(sqllist);
            if (rowsAffected > 0)
            {
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_article GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_article GetModel(string call_index)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        public DataSet GetList(string channel_name, int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" * FROM view_" + channel_name);
        ...
        public int GetCount(string channel_name, int category_id, string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ");
            strSql.Append(" count(1) FROM view_" + channel_name);
            strSql.Append(" where datediff(d,add_time,getdate())>=0");
            if (category_id > 0)
            {
                strSql.Append(" and category_id in(select id from " + databaseprefix + "article_category where class_list like '%," + category_id + ",%')");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM view_" + channel_name);
            strSql.Append(" where datediff(d,add_time,getdate())>=0");
            if (category_id > 0)
            {
                strSql.Append(" and category_id in(select id from " + databaseprefix + "article_category where class_list like '%," + category_id + ",%')");
        ...
        /// 获得会员组价格
        /// </summary>
        private List<Age_user_group_price> GetGroupPrice(int article_id)
        {
            List<Age_user_group_price> ls = new List<Age_user_group_price>();

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,article_id,group_id,price from " + databaseprefix + "user_group_price ");
            strSql.Append(" where article_id=" + article_id);
            DataTable dt = DbHelperMSSQL.Query(strSql.ToString()).Tables[0];

            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                Age_user_group_price model;
                for (int n = 0; n < rowsCount; n++)
                {
                    model = new Age_user_group_price();

...
namespace Angelcms.DAL
{
    ...
    public partial class channel
    {
        ...
        /// 增加一条数据及其子表
        /// </summary>
        public int Add(Age_channel model)
        {
            using (SqlConnection conn = new SqlConnection(DbHelperMSSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        object obj = DbHelperMSSQL.GetSingle(conn, trans, strSql.ToString(), parameters); //带事务
                        ...
                        if (model.channel_fields != null)
                        {
                            ...
                            foreach (Age_channel_field modelt in model.channel_fields)
                            {
                                ...
                                DbHelperMSSQL.ExecuteSql(conn, trans, strSql2.ToString(), parameters2);
                            }
                        }
                        ...
                        if (model.channel_fields != null)
                        {
                            foreach (Age_channel_field modelt in model.channel_fields)
                            {
                                Age_article_attribute_field fieldModel = new DAL.article_attribute_field(databaseprefix).GetModel(modelt.field_id);
                                if (fieldModel != null)
                                {
                        ...
                        DbHelperMSSQL.ExecuteSql(conn, trans, strSql3.ToString());
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Age_channel model)
        {
            Age_channel oldModel = GetModel(model.id); //旧的数据
            using (SqlConnection conn = new SqlConnection(DbHelperMSSQL.connectionString))
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (model.channel_fields != null)
                        {
                            ...
                            {
                                strSql2 = new StringBuilder();
                                Age_channel_field fieldModel = null;
            ...
            using (SqlConnection conn = new SqlConnection(DbHelperMSSQL.connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        DbHelperMSSQL.ExecuteSql(conn, trans, strSql1.ToString());
                        ...
                        DbHelperMSSQL.ExecuteSql(conn, trans, strSql3.ToString(), parameters3);
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_channel GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            Age_channel model = new Age_channel();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                DataSet ds2 = DbHelperMSSQL.Query(strSql2.ToString(), parameters2);
                if (ds2.Tables[0].Rows.Count > 0)
                {
                    int i = ds2.Tables[0].Rows.Count;
                    List<Age_channel_field> models = new List<Age_channel_field>();
                    Age_channel_field modelt;
                    for (int n = 0; n < i; n++)
                    {
                        modelt = new Age_channel_field();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Age_channel GetModel(SqlConnection conn, SqlTransaction trans, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,category_id,name,title,is_albums,is_attach,is_group_price,page_size,sort_id from " + databaseprefix + "channel ");
            strSql.Append(" where id=@id ");
            ...
            Age_channel model = new Age_channel();
            DataSet ds = DbHelperMSSQL.Query(conn, trans, strSql.ToString(), parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                #region  父表信息
                if (ds.Tables[0].Rows[0]["id"].ToString() != "")
                ...
                DataSet ds2 = DbHelperMSSQL.Query(conn, trans, strSql2.ToString(), parameters2);
                if (ds2.Tables[0].Rows.Count > 0)
                {
                    int i = ds2.Tables[0].Rows.Count;
                    List<Age_channel_field> models = new List<Age_channel_field>();
        ...
        public int GetChannelId(string channel_name)
        {
            ...
            string str = Convert.ToString(DbHelperMSSQL.GetSingle(strSql.ToString(), parameters));
        ...
        public string GetChannelName(int id)
        {
            ...
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
            string name = Convert.ToString(DbHelperMSSQL.GetSingle(strSql.ToString(), parameters));
        ...
        /// 根据频道的名称查询ID
        /// </summary>
        public Age_channel GetModel(string channel_name)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 id,category_id,name,title,is_albums,is_attach,is_group_price,page_size,sort_id from " + databaseprefix + "channel ");
            strSql.Append(" where name=@name ");
        ...
        public int GetPageSize(string channel_name)
        {
            ...
            return Convert.ToInt32(DbHelperMSSQL.GetSingle(strSql.ToString(), parameters));
        }
        /// <summary>
        /// 删除已移除的频道扩展字段
        /// </summary>
        public void FieldDelete(SqlConnection conn, SqlTransaction trans, List<Age_channel_field> models, int channel_id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select id,field_id from " + databaseprefix + "channel_field");
            strSql.Append(" where channel_id=" + channel_id);
            DataSet ds = DbHelperMSSQL.Query(conn, trans, strSql.ToString());
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                Age_channel_field model = models.Find(p => p.field_id == int.Parse(dr["field_id"].ToString())); //查找对应的字段ID
                if (model == null)
                {
                    DbHelperMSSQL.ExecuteSql(conn, trans, "delete from " + databaseprefix + "channel_field where channel_id=" + channel_id + " and field_id=" + dr["field_id"].ToString()); //删除该字段
                }
            }
        }
        /// <summary>
        /// 删除及重建该频道视图
        /// </summary>
        public void RehabChannelViews(SqlConnection conn, SqlTransaction trans, Age_channel model, string old_name)
        {
            //删除旧的视图
            StringBuilder strSql1 = new StringBuilder();
            ...
            DbHelperMSSQL.ExecuteSql(conn, trans, strSql2.ToString());
        }
        #endregion
    }
}